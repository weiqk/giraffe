...
void CaptureNetPacket::PacketHandler(unsigned char *param, const struct pcap_pkthdr *header, const unsigned char *pkt_data)
{
	...
	item.header = *header;
	unsigned char *pdata;
	if(static_cast<unsigned int >(FLAGS_CAP_PACK_BUF_SIZE)< header->caplen)
	{
	...
	{
		pdata = new unsigned char[FLAGS_CAP_PACK_BUF_SIZE];
		memset(pdata, 0, FLAGS_CAP_PACK_BUF_SIZE);
